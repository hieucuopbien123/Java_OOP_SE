...
public class Utils {
	...
	public static double calculateFee(long interval, int biketype, int wheelType) {
    	double factor = 1;
        if((biketype == 0 && wheelType == 2) || (biketype == 1 && wheelType == 1)) {
        	factor = 1.5;
        } else if(biketype == 1 && wheelType == 2) {
        	factor = 2;
        }
        double tempPrice = factor*10000;
        if(interval > 1800) {
        	long temp1 = (interval - 1800)/900;
        	long temp2 = (interval - 1800)%900 > 1 ? 1 : 0;
        	
        	tempPrice += factor*(temp1 + temp2)*3000;
        }
        return tempPrice;
	}
}